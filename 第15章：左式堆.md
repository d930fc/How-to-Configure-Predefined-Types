...
每个节点可以有0个儿、1个儿或2个儿。左式堆不需要是完全二叉树。左式堆中任意节点的值小于这个节点的儿的值。左式堆中任意节点的大儿的零路径长大于等于小儿的零路径长。从节点到具有0个儿或1个儿的节点的最少边数称为这个节点的零路径长。

插入称为insert，删除最小元称为deleteMin，查找最小元称为findMin，合并称为merge。
