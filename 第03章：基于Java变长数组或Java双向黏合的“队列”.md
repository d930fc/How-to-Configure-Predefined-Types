# 基于Java变长数组或Java双向黏合的“队列”

### 队列

形如A<sub>0</sub>，A<sub>1</sub>，A<sub>2</sub>，…，A<sub>N-1</sub>的有序集合，规定插入操作和删除操作只能分别固定发生在一端，这种数据结构称为队列，执行插入的那一端称为队尾，执行删除的那一端称为队头。插入称为enqueue，删除称为dequeue。
